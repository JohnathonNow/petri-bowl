    ...
    for _ in range(CONSTANTS["PRETRAIN_EPOCHS"]):
        ...
        for _ in range(CONSTANTS["PRETRAIN_STEPS"]): # max steps per pre-train epoch
            ...
            for i in range(6):
                ...
                if i == 0:
                    target_x, target_y = CONSTANTS["EXPERT_TARGET_X_1_G"], env.ball_pos[1]
                else:
                    if i in [1, 2]:
                        target_x, target_y = min(env.ball_pos[0], CONSTANTS["EXPERT_TARGET_X_1_D_MAX"]), env.ball_pos[1]
                    else:
                        target_x, target_y = env.ball_pos[0], env.ball_pos[1]
                ...
                if i == 0:
                    target_x, target_y = CONSTANTS["EXPERT_TARGET_X_2_G"], env.ball_pos[1]
                else:
                    if i in [1, 2]:
                        target_x, target_y = max(env.ball_pos[0], CONSTANTS["EXPERT_TARGET_X_2_D_MIN"]), env.ball_pos[1]
                    else:
                        target_x, target_y = env.ball_pos[0], env.ball_pos[1]
