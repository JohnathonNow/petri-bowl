...
class SoccerEnv:
    ...
    def step(self, actions1, actions2):
        ...
        old_ball_x = self.ball_pos[0]
        self.ball_pos[0] += self.ball_vel[0]
        self.ball_pos[1] += self.ball_vel[1]

        # friction
        self.ball_vel[0] *= CONSTANTS["BALL_FRICTION"]
        self.ball_vel[1] *= CONSTANTS["BALL_FRICTION"]

        self._apply_bounds(self.ball_pos, self.ball_vel)
        ...
        if not getattr(self, 'pending_replay', False):
            scored_goal = False
            if old_ball_x > CONSTANTS["GOAL_X_1"] and self.ball_pos[0] <= CONSTANTS["GOAL_X_1"]:
                if goal_y_min <= self.ball_pos[1] <= goal_y_max:
                    self.score[1] += 1
                    scored_goal = True
            elif old_ball_x < CONSTANTS["GOAL_X_2"] and self.ball_pos[0] >= CONSTANTS["GOAL_X_2"]:
                if goal_y_min <= self.ball_pos[1] <= goal_y_max:
                    self.score[0] += 1
                    scored_goal = True
            ...
            if scored_goal:
                ...
                self._resolve_net_collision(self.ball_pos, self.ball_vel, True)
        else:
            self._resolve_net_collision(self.ball_pos, self.ball_vel, True)
            ...
            if self.steps >= self.goal_scored_step + CONSTANTS["REPLAY_DELAY"]:
                ...
                for state_snapshot in replay_history:
                    ...
                    self.ball_pos = state_snapshot["ball_pos"]
        ...
        def check_stick_collision(p_pos, p_angle):
            ...
            px_v, py_v = ball_x - px, ball_y - py

            # Project ball onto stick line
            ...
            if dist < CONSTANTS["BALL_RADIUS_THRESH"]:
                if dist == 0:
                    self.ball_vel[0] += random.choice([-1.0, 1.0])
                    self.ball_vel[1] += random.choice([-1.0, 1.0])
                else:
                    self.ball_vel[0] += (dx/dist) * 2.0
                    self.ball_vel[1] += (dy/dist) * 2.0
    ...
    def _resolve_net_collision(self, pos, vel, is_ball):
        # Team 1 net
        if CONSTANTS["NET1_MIN_X"] <= pos[0] <= CONSTANTS["NET1_MAX_X"] and CONSTANTS["GOAL_Y_MIN"] <= pos[1] <= CONSTANTS["GOAL_Y_MAX"]:
            ...
            if min_d == dx1:
                ...
                vel[0] = -vel[0] if is_ball else 0.0
            elif min_d == dx2:
                pos[0] = CONSTANTS["NET1_MAX_X"]
                vel[0] = -vel[0] if is_ball else 0.0
            elif min_d == dy1:
                pos[1] = CONSTANTS["GOAL_Y_MIN"]
                vel[1] = -vel[1] if is_ball else 0.0
            else:
